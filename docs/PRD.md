...
- Over http and https the address bar shows the view's share address rather than its hash route: the site root for home and `p/<slug>.html` for a prompt. Added in v1.46.0 so that the address a reader copies is one a link preview can read. On `file://` the hash route stays, because the browser does not allow the rewrite there. See section 32a
...
Since v1.46.0 each prompt has one live address of its own, its share page in `p/`, so removing a prompt now includes one retirement. The share page is not deleted. It is rewritten as a retired page that forwards to the site root, and it stays published. Everything else about the prompt is still source and is still pruned.
...
The `hidden: true` flag remains supported for retiring a prompt from navigation without deleting it. As of v1.19.0 no prompt uses it. A hidden prompt gets no share page, so hiding one also means retiring its share page, forwarding to `index.html#/<slug>`, where the prompt is still reachable.

---

## 13. Repository Structure

The whole project is 22 files in six folders. There is no build output, no vendored code, no ignored directory, and no ignore file: `.gitignore`, `.editorconfig`, and `.vscode/` are all absent, so every file in the working tree is tracked.
...
├── prompts/            Six .md files, one per prompt. The readable source.
├── p/                  Six generated share pages, one per visible prompt.
│                       Public addresses: retired, never deleted. Section 32a.
├── tools/
│   └── prompts-mirror.py  Maintenance only. Checks or resyncs the mirror
│                       and writes the share pages.
...
- Prompt text must never instruct its reader to push, commit, or publish to a remote (PRD 11). Verified clean across all six prompts, four on 2026-08-23, and `prompt-audit` and `brand-identity` on 2026-09-23.
...
- Adding a prompt touches four places in order: the `.md` file, `js/prompts-data.js`, the README prompt list, and `docs/PATCHNOTES.md` (PRD 12, README). The share page is written by the same `--sync` that writes the data file.
...
- Every visible prompt has a share page at `p/<slug>.html`, generated by `tools/prompts-mirror.py` and never edited by hand. A share page is a public address: once published it is never deleted, only rewritten as a retired page that forwards in one hop (PRD 32a).
...
| Layout shell, script order, meta tags | `index.html`, all 51 lines of it |
...
| Tags or categories | Same reason. Six prompts do not need a taxonomy, and one imposed early tends to outlive its usefulness |
...
| Per-prompt HTML pages | Would make adding a prompt an HTML task. The single shell is the core architectural choice. The share pages added in v1.46.0 are not this: they hold no content, are generated rather than written, and exist only so a link preview can describe a prompt |
...
- A skip-to-content link, so a keyboard user reaching a prompt page does not pass nine focus stops before the copy button. See the accessibility section of `docs/DESIGN.md`. This is now the largest known gap in the project.
...
The Documentation prompt reached its current shape in v1.37.0, and this project has not been re-audited against it since v1.27.0. Six sections were added to the prompt in the interval: Browser Testing, Verification Environment, Licensing, Social Sharing Tags, and the folder structure rules for `LICENSE.md`, `robots.txt`, and `sitemap.xml`. This site satisfies some of them and not others.

A partial read-only pass was run on 2026-09-07 and stopped before any file was written. Its findings are recorded here so the work does not have to be rediscovered. They are findings, not decisions, and each is stated with the reasoning that produced it.

**1. `robots.txt` should not be created in this repository, and the standard is what says so.** The site is served from `https://azqato.github.io/prompts/`, a subdirectory of a larger domain rather than an origin of its own. A `robots.txt` at this repository's root deploys to `/prompts/robots.txt`, and a crawler reads robots.txt from the origin root and nowhere else, so the file would be exactly the unreferenced text file the prompt warns about. The governing file is `https://azqato.github.io/robots.txt`, which belongs to a different repository and returned 404 when checked on 2026-09-07. The prompt's instruction for this case is to say so in the PRD rather than to create the file, which is what this paragraph does.

**2. `sitemap.xml` is valid here but would carry a single URL.** A sitemap is scope-limited by its own location, and one at `/prompts/sitemap.xml` may list any URL under `/prompts/`, which is the whole of this site. So the position is correct and the file would be legitimate. The limit is the architecture: every route is a fragment on one `index.html`, and a crawler never sees a fragment, so the only entry a sitemap could honestly hold is `https://azqato.github.io/prompts/` itself. Nothing currently points at such a file either, since the `Sitemap:` line that would cross-submit it lives in the root robots.txt that does not exist. Worth doing for correctness, not for discovery.

**3. Social sharing tags are absent, and adding them runs into a conflict between two rules this project holds.** `index.html` carries a `<meta name="description">` and nothing else, so a pasted link renders from the title tag alone. The blocker is not the tags themselves. The Social Sharing Tags default asks for a description written in a marketing-focused tone, and section 11 of this document prohibits marketing language outright. Under the prompt's own rule that every policy yields to a rule the project already states, section 11 wins and the description is written plain, with the difference flagged rather than silently resolved. That is the correct outcome here, and it is also a signal worth acting on in the prompt itself: a default that contradicts the writing style in the same document will produce this collision on any project that has a house style. See open question 8 for the two things that have to be settled before the tags can be written at all, the canonical domain and how a hash-routed single page satisfies a rule that wants `og:url` unique per page. *Corrected in v1.44.0:* the conflict described here does not exist. The Social Sharing Tags section of the Documentation prompt has asked for an educational tone, complete sentences stating what the page does, since it was added in v1.37.0, and "marketing-focused" appears only in this paragraph. Section 11 and the default agree. The remaining blockers are the two in open question 8, and both are taken up in the per-prompt sharing item below. *Resolved in v1.46.0* by section 32a.
...
**Goal.** A link to a prompt, pasted into a chat app or a social post, renders a card with that prompt's title and a short summary of what it does. A link to the home page renders a card for the site.

**What happens today.** Observed on 2026-09-23, when `https://azqato.github.io/prompts/#/prompt-audit` was pasted into Discord: the card read "Azqato's Prompts" over "A personal library of reusable Claude Code prompts." Every prompt link produces that same card. It is built from the `<title>` and `<meta name="description">` in `index.html`, because the page carries no Open Graph or Twitter Card tags at all.

**Why adding tags to `index.html` is not enough.** A link renderer requests the URL without its fragment, since the part after `#` is never sent to a server, and it does not run JavaScript. To a renderer, every route on this site is the same `index.html`, so no set of tags in that file can describe one prompt. The blocker is the hash routing, not the missing tags. Tags on `index.html` fix the home card only.

**The standard to follow.** This PRD states no social sharing policy of its own. Open question 8 has been open since v1.37.0. So the default in the Social Sharing Tags section of the Documentation prompt applies, as the prompt's own rule for a project without one directs:

- All six tags on every shareable page: `og:title`, `og:description`, `og:url`, `og:type` (`website`), `og:site_name`, and `twitter:card`.
- `og:title` is the prompt's title and does not repeat the site name. The current titles run 10 to 13 characters, well inside the 60 target.
- `og:site_name` is "Azqato's Prompts", 16 characters against a budget of 20.
- `og:url` is the absolute https address of that prompt's own page, unique per prompt, never the site root. The canonical address is `https://azqato.github.io/prompts/`, recorded in section 17, which settles the first half of open question 8.
- `og:description` is complete sentences stating what the prompt does, 150 characters as the target and 200 the ceiling, written to end on a full stop rather than be cut.
- No images: `twitter:card` is `summary`, and no `og:image` is declared.
- Pages excluded from sharing, such as any prompt carrying the `hidden` flag, are listed in this section so the exclusion reads as a decision.
- Compliance is checked with the Documentation prompt's list, run against the generated files rather than argued about.

**Proposed approach.** One static share page per prompt at a real path, with the six tags in its head and a `<meta http-equiv="refresh">` sending a visitor on to `index.html#/<slug>`, plus a plain link for any client that ignores refresh. A renderer reads the tags and stops, and a person lands on the prompt. A meta refresh rather than a script, because the Content Security Policy forbids inline script and because it also works from `file://`. The pages are generated by `tools/prompts-mirror.py` from each prompt's frontmatter, and checked by its verification mode, so they are derived rather than hand-written and cannot drift from the prompts. That keeps Python as the only tool in the loop and adds no dependency. `index.html` gets the same six tags describing the site. The brand suffix on page titles, finding 5 of the self-audit above, touches the same code and can ship with this.

**Decisions needed first.**

1. **The path shape**, such as `p/<slug>.html` or `<slug>/`. This is the one decision that is hard to reverse. Once a share URL has been pasted anywhere, it is a public address under section 32, so a later rename or removal of a prompt has to retire its share page behind a redirect rather than delete it.
2. **Where the short description comes from.** The frontmatter descriptions run 140 to 202 characters. Four of the five are over the 150 target, and GitHub Wiki's 202 is over the 200 ceiling. Either the existing descriptions are tightened to fit, which the standard prefers because it reuses one accurate description rather than inventing a competing second one, or a separate short field is added to the frontmatter for sharing. Tightening also changes the home page cards, which display the same text.
3. **How the share URL reaches the person sharing.** The address bar will still show the hash URL, and that is what people copy, so share pages alone change nothing about what gets pasted. Options include a "Copy link" control beside the copy button, or rewriting the address bar to the share path with `history.replaceState` on https, which `file://` does not permit and so would need a guard.

**Verification.** Locally: open each generated share page from disk and confirm it lands on the right prompt, and run the Documentation prompt's tag checks over the generated files. After the deploy: fetch a share page and confirm its tags match the local copy, then paste one prompt link and the home link into Discord and confirm each card shows the right title and description.
...
| Requests | Four, all same-origin: the HTML, one stylesheet, two scripts. A visitor arriving from a pasted share link first loads the 21-line share page, which requests nothing and forwards | No additional request may be added. A fifth would mean a dependency |
...
Verify after a deploy by loading `https://azqato.github.io/prompts/` and checking the home list, one prompt page, and the copy button. Since v1.46.0, also fetch one share page and confirm it matches the local file. The local `file://` check and the deployed check are not redundant: they differ in protocol and in base path.
...
There is no development mode, no feature flag, and no configuration file. The one piece of conditional behaviour, since v1.46.0, is the address bar rewrite, which runs over http and https and is skipped on `file://`, where the browser forbids it. Every environment runs byte-identical assets, which is why a local check is a valid check.
...
| --- | --- | --- |
| Markup | HTML5 | Living standard. `index.html`, 51 lines, plus six generated share pages of 21 lines each |
...
```
/
├── index.html              51 lines. The only page with content. Carries the Content
...
Twenty-two files, six folders, two levels deep at most. No build output, no vendored code, no ignored directory, and no ignore file: `.gitignore`, `.editorconfig`, `.github/`, and `.vscode/` are all absent, so every file in the working tree is tracked.
...
2. `window.location`, which is the single source of truth for the current view: the hash when there is one, otherwise a `p/<slug>.html` path. Since v1.46.0 the router also writes it, through `history.replaceState()`, which replaces the current history entry rather than adding one.
...
The share pages in `p/` carry a stricter policy of their own, `default-src 'none'; base-uri 'none'; form-action 'none'`, because they load nothing at all: no script, no style, no image. A meta refresh is a navigation rather than a fetch, so the policy does not block it. Verified in v1.46.0 by opening them from `file://` and over http and landing on the prompt both times.
...
Added in v1.46.0. Numbered `32a` rather than inserted as a new section 33, for the reason section 10a gives. This is the project's own policy, which the Documentation prompt asks a project that serves a site to state. Until v1.46.0 there was none, and open question 8 recorded the gap.
